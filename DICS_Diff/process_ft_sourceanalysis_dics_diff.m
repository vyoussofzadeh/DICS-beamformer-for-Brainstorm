function varargout = process_ft_sourceanalysis_dics_diff(varargin )
...
sProcess.Comment     = 'FieldTrip: ft_sourceanalysis (DICS) diff';
...
% sInputs = sInputsA;
Data_A = do_read_sInputs (sInputsA, Modality);
Data_B = do_read_sInputs (sInputsB, Modality);

ftLeadfield = Data_B.ftLeadfield;
ftHeadmodel = Data_B.ftHeadmodel;
HeadModelFile = Data_B.HeadModelFile;
HeadModelMat = Data_B.HeadModelMat;
DataMat  = Data_B.DataMat;
iChannelsData  = Data_B.iChannelsData;
DataFile  = Data_B.DataFile;
...
cfg.toi        = PostStim(1):0.05:PostStim(2);
tfrA            = ft_freqanalysis(cfg, Data_A.ftData);
tfrB            = ft_freqanalysis(cfg, Data_B.ftData);

% Plot TFR
cfg = [];
cfg.fmax = MaxFreq;
cfg.toi = [PostStim(1), PostStim(2)];
cfg.baselinetime = [PostStim(1), PostStim(2)];
cfg.plotflag = ShowTfr;
[time_of_interest,freq_of_interest] = do_tfr_plot(cfg, tfrA, tfrB);
% [time_of_interest,freq_of_interest] = do_tfr_plot(cfg, tfrB);
...
cfg = [];
cfg.toilim = PostStim;
ep_data.bsl = ft_redefinetrial(cfg, Data_A.ftData);
% Post-stim
cfg.toilim = PostStim;
ep_data.pst = ft_redefinetrial(cfg, Data_B.ftData);
% Baseline + Post-stim
cfg = [];
...
switch sProcess.options.sensortype.Value{1}
    case {'EEG', 'SEEG', 'ECOG'}
        cfg_main.sens = Data_A.ftData.elec;
    case {'MEG', 'MEG GRAD', 'MEG MAG'}
        cfg_main.sens = Data_A.ftData.grad;
end
...
%% ===== TIME-FREQUENCY =====
function [time_of_interest,freq_of_interest] = do_tfr_plot(cfg_main, tfrA, tfrB)
% First compute the average over trials:

cfg = [];
freq_avg_A = ft_freqdescriptives(cfg, tfrA);
freq_avg_B = ft_freqdescriptives(cfg, tfrB);

freq_avg_A.powspctrm(isnan(freq_avg_A.powspctrm))=0;
meanpow_A = squeeze(mean(freq_avg_A.powspctrm, 1));

freq_avg_B.powspctrm(isnan(freq_avg_B.powspctrm))=0;
meanpow_B = squeeze(mean(freq_avg_B.powspctrm, 1));

meanpow = meanpow_B - meanpow_A;
...
function out = do_read_sInputs (sInputs, Modality)
...
out = [];
out.iChannelsData = iChannelsData;
out.ChannelMat = ChannelMat;
out.HeadModelFile = HeadModelFile;
out.HeadModelMat = HeadModelMat;
out.ftHeadmodel = ftHeadmodel;
out.ftLeadfield = ftLeadfield;
out.ftData = ftData;
out.AllChannelFiles = AllChannelFiles;
out.iChanInputs = iChanInputs;
out.DataMat = DataMat;
