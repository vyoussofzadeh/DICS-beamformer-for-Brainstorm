function out = vy_mri_neuromag6(cfg_main)
...
%     nii_filename_final = fullfile(cfg_main.outputmridir,'T1.nii');
%     cfg                 = [];
...
%     
%     biasfield = spm_bias_estimate(nii_filename_final);
%     
% %     addpath(fullfile(allpath.ft_path,'/external/spm8'));
%     spm_bias_apply(nii_filename_final, biasfield);
%     
%     nii_filename_final_bc = fullfile(cfg_main.outputmridir,'mT1.nii');
%     MRI_BC = ft_read_mri(nii_filename_final_bc);
...
%     ft_sourceplot([], mri_realigned1);
    ...
    brain = ft_read_mri('/MEG_data/MRI_database/epilepsy/CORCORAN_Margaret/corcoran_margaret_09_05_2019_FSRecon/mri/brain.mgz');
    brain1 = ft_transform_geometry(mri_realigned.transformorig,brain);
    brain2 = ft_transform_geometry(mri_realigned.transform,brain1);
    brain3 = ft_transform_geometry(mri_realigned.transform/mri_realigned.transformorig/brain.transform,brain);
    ft_sourceplot([], brain3);
    
%     
    brain1 = [];
    brain1.brain  = brain.anatomy;
    brain1.transform = mri_realigned.transform;
    ...
    cfg.spmversion = 'spm12';
    individual_headmodel = ft_prepare_headmodel(cfg, brain1);

    figure;
    ft_plot_vol(individual_headmodel, 'facecolor', 'cortex', 'edgecolor', 'none');alpha 0.5; camlight;
    individual_headmodel1 = ft_transform_geometry(mri_realigned.transform,individual_headmodel);
    
    %%
    SurfaceFile = '/MEG_data/epilepsy/corcoran_margaret/brainstorm_db/anat/corcoran_margaret/tess_cortex_pial_low.mat'
    sourcemodel = ft_read_headshape(SurfaceFile);
    
    cfg = [];
    cfg.method = 'singleshell';
    cfg.spmversion = 'spm12';
    individual_headmodel = ft_prepare_headmodel(cfg, sourcemodel);
    figure;
    ft_plot_vol(individual_headmodel, 'facecolor', 'cortex', 'edgecolor', 'none');alpha 0.5; camlight;
