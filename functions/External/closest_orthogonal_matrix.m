...
iter = 0;
A_b  = conj(A);
d    = sqrt(dot( A, A_b, 1 )); % alternative: ones( 1, cols(A) )
rho  = NaN(MAX_ITER, 1);
...
while iter < MAX_ITER,
    ...
    try
        V = symmetric_orthogonalise(scale_cols( A, d ));
    catch ME
...
if nargout > 3,
    ...
    [~, ~, ~, W] = symmetric_orthogonalise(scale_cols( A, d ));
    W = diag(d) * W * diag(d);
end%if
...
% % % [L, d, rhoFinal] = closest_orthogonal_matrix(A);
